...
NUM_EXPERIMENT = 10  # number of independent random-initial-solution samplings
REPS = 1
PROBLEM_SIZE = 7
NUM_ITERATIONS = 10  # impact-reordering decomposition rounds per sampling
...
def save_trained_circuits_and_initial_solutions():
    """
    Train IGDec-QAOA in ideal noiseless simulation and save the elevator_three
    circuits and initial solutions (same cost+pcount+dist QUBO formulation as
    elevator_two; an independently-trained circuit set/repetition kept as a
    separate combo per the study's combo matrix).

    NOT executed automatically (run
    `python loch_qaoa_elev_three_extract_circuits.py train` on the machine that
    does the actual training) and NOT run in this authoring session.

    Compute estimate (elevator, unfiltered, 1,925 rows):
    each of the NUM_EXPERIMENT=10 samplings runs NUM_ITERATIONS=10
    impact-reordering iterations; each iteration re-solves subproblems
    covering the top 15% of the dataset by impact, i.e.
    floor(0.15 * 1925 / 7) = 41 seven-qubit subproblems per iteration. That is
    10 * 10 * 41 = ~4,100 ideal-simulator QAOA subproblem-solves (COBYLA(500)
...
def run_hardware_like_from_saved_circuits():
    ...
    raw_counts_path = os.path.join(results_dir, f"{RUN_LABEL}-raw_counts.jsonl")
    raw_counts_writer = RawCountsWriter(raw_counts_path)

    trex_raw_counts_path = os.path.join(results_dir, f"{RUN_LABEL}-trex-raw_counts.jsonl")
    trex_counts_writer = RawCountsWriter(trex_raw_counts_path)

    # Resolved once from the real budget planner (see
    # piastq_execution.budget_planner.resolve_all_combo_settings): this
    ...
    settings = resolve_all_combo_settings()["elevator_o3_igdec_qaoa"]
    ...
    # Whether TREx runs *at all* is decided by settings.trex_enabled below,
    # since the plan may have dropped it.
    trex_twirl_instances = load_trex_twirl_instances()
    trex_rng = random.Random()

    for sampling_id in range(1, settings.repetitions + 1):
        ...
        while count < NUM_ITERATIONS:
            ...
            if PROBLEM_SIZE > 0.15 * len(df):
                ...
                counts, raw_record = run_circuit_with_batching_recorded(
                    ...
                    dataset=RUN_LABEL,
                    circuit_id=f"{RUN_LABEL}_sampling{sampling_id}_itr{meta['iteration']}_sub{meta['subproblem_index']}",
                    backend=backend,
                ...
                # TREx: a distinct hardware pass per twirl instance -- cannot
                # reuse the raw counts above. Written to a separate file
                # since it's mitigation data, not the raw baseline;
                # evaluation-time undoing/aggregation is
                # piastq_execution.mitigation.aggregate_trex_records().
                # Skipped entirely if the plan dropped TREx for this combo.
                if settings.trex_enabled:
                    for twirl_idx in range(trex_twirl_instances):
                        ...
                        _trex_counts, trex_record = run_circuit_with_batching_recorded(
                            ...
                            objective_mode="single_objective",
                            dataset=RUN_LABEL,
                            circuit_id=f"{RUN_LABEL}_sampling{sampling_id}_itr{meta['iteration']}_sub{meta['subproblem_index']}_trex{twirl_idx}",
                            backend=backend,
                            iteration_id=meta["iteration"],
                            subproblem_id=meta["subproblem_index"],
                            shots_per_batch=settings.shots_per_batch,
    ...
    print(f"Saved TREx raw counts to: {trex_raw_counts_path}")


if __name__ == "__main__":
    # `python loch_qaoa_elev_three_extract_circuits.py train` (re)trains
    # elevator_three's IGDec-QAOA circuits on an ideal simulator -- this must
    # run on the machine that does the actual training.
